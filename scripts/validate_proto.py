...
try:
    ...
    print("[FAIL] Could not import generated stubs.")
...
try:
    ...
    print("[FAIL] Failed creating channel:", e)
    sys.exit(1)
...
try:
    ...
    print("[OK] Stubs instantiated")
except Exception as e:
    print("[FAIL] Stub instantiation failed:", e)
    sys.exit(1)


print("\n=== Checking RPC Methods ===")

check(hasattr(ingest_stub, "PushMeasurements"), "PushMeasurements RPC exists")
check(hasattr(agg_stub, "StreamAggregates"), "StreamAggregates RPC exists")
check(hasattr(query_stub, "GetSensorStats"), "GetSensorStats RPC exists")
...
check(hasattr(telemetry_pb2.Measurement(), "value"), "Measurement.value field exists")
check(hasattr(telemetry_pb2.Aggregate(), "count"), "Aggregate.count field exists")
check(
    hasattr(telemetry_pb2.GetSensorStatsResponse(), "recent"),
    "GetSensorStatsResponse.recent field exists",
)

print("\n[OK] Proto validation passed.")
