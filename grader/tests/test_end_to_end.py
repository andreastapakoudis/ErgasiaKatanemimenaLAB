...
def test_client_streaming_ingest_and_redis_side_effects(ingest_stub, redis_client):
    """
    Validates client-streaming RPC + Redis aggregation keys.
    We inject deterministic measurements and verify:
      - global agg:{type}:{location} stats
      - per-sensor sensor:{id}:stats
      - bounded recent list sensor:{id}:recent (trimmed to <=20)
    """
    # deterministic data: 2 sensors, same type/location, known values
    sensor_id = "T-01"
...
def test_server_streaming_aggregates_emits_updates(aggregate_stub):
    """
    Validates server-streaming RPC delivers at least on Aggregate update.
    (We already ingested in previous test; initial snapshot should arrive.)
    """
    ...
    # Expect at least one message quickly
    first = next(stream)
...
def test_unary_query_returns_stats_and_recent(query_stub):
    """
    Validates unary RPC QueryService.GetSensorStats.
    """
...
def test_fastapi_http_endpoint(http_session):
    """
    Validates FastAPI bridge exposes HTTP endpoint that calls unary gRPC internally.
    """
...
# def test_fastapi_ws_receives_broadcasts(http_session):
#     import asyncio
...
@pytest.mark.asyncio
async def test_fastapi_ws_receives_broadcasts(http_session):
    async with websockets.connect("ws://api:8000/ws", open_timeout=5) as ws:
        # await ws.send("ping")
        msg = await asyncio.wait_for(ws.recv(), timeout=STREAM_TIMEOUT_S)

    assert isinstance(msg, str) and len(msg) > 0
    assert "placeholder" not in msg, "Fake message received"
